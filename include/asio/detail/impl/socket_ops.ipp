...
size_t sync_recv(socket_type s, state_type state, buf* bufs,
    ...
    for (;;) {
        ...
        // Operation failed.
        if ((state & user_set_non_blocking) || (ec != asio::error::would_block && ec != asio::error::try_again))
            return 0;
...
int setsockopt(socket_type s, state_type& state, int level, int optname,
    ...
    get_last_error(ec, result != 0);
    if (result == 0) {
#if defined(__MACH__) && defined(__APPLE__) || defined(__NetBSD__) || defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__QNX__)
    ...
    if (HMODULE winsock_module = ::GetModuleHandleA("ws2_32")) {
        ...
        if (gso_t gso = (gso_t)::GetProcAddress(winsock_module, "getsockopt")) {
            ...
            *optlen = static_cast<size_t>(tmp_optlen);
            if (result != 0 && level == IPPROTO_IPV6 && optname == IPV6_V6ONLY && ec.value() == WSAENOPROTOOPT && *optlen == sizeof(DWORD)) {
...
inline int getaddrinfo_emulation(const char* host, const char* service,
    ...
    for (gai_search* sptr = search; sptr < search + search_count; ++sptr) {
        ...
        if (host != 0 && host[0] != '\0' && hptr->h_name && hptr->h_name[0] && (hints.ai_flags & AI_CANONNAME) && canon == 0) {
...
u_long_type network_to_host_long(u_long_type value) {
    ...
    u_long_type result = (static_cast<u_long_type>(value_p[0]) << 24) | (static_cast<u_long_type>(value_p[1]) << 16) | (static_cast<u_long_type>(value_p[2]) << 8) | static_cast<u_long_type>(value_p[3]);
    return result;
#else   // defined(ASIO_WINDOWS_RUNTIME)
    return ntohl(value);
#endif  // defined(ASIO_WINDOWS_RUNTIME)
