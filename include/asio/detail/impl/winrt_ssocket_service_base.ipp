        ...
        switch (static_cast<const socket_addr_type*>(addr)->sa_family) {
            case ASIO_OS_DEF(AF_INET):
                if (addr_len < sizeof(sockaddr_in4_type)) {
                    ...
                    reinterpret_cast<sockaddr_in4_type*>(addr)->sin_port = socket_ops::host_to_network_short(port);
                    ec = asio::error_code();
            ...
            case ASIO_OS_DEF(AF_INET6):
                if (addr_len < sizeof(sockaddr_in6_type)) {
                    ...
                    reinterpret_cast<sockaddr_in6_type*>(addr)->sin6_port = socket_ops::host_to_network_short(port);
                    ec = asio::error_code();
