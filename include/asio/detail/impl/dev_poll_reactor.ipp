...
void dev_poll_reactor::run(long usec, op_queue<operation>& ops) {
    ...
    if (usec == 0 && op_queue_[read_op].empty() && op_queue_[write_op].empty() && op_queue_[except_op].empty() && timer_queues_.all_empty())
        return;
    ...
    for (int i = 0; i < num_events; ++i) {
        ...
        if (descriptor == interrupter_.read_descriptor()) {
            ...
            if ((events[i].events & (POLLERR | POLLHUP)) != 0 && !more_except && !more_reads && !more_writes) {
