...
struct set_value_free_trait<T, void(Vs...),
                            typename void_type<
                                decltype(set_value(declval<T>(), declval<Vs>()...))>::type> {
...
struct set_value_free_trait<T, void(),
                            typename void_type<
                                decltype(set_value(declval<T>()))>::type> {
    ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
...
#define ASIO_PRIVATE_SET_VALUE_FREE_TRAIT_DEF(n)                                                          \
    ...
    struct set_value_free_trait<T, void(ASIO_VARIADIC_TARGS(n)),                                          \
                                typename void_type<                                                       \
                                    decltype(set_value(declval<T>(), ASIO_VARIADIC_DECLVAL(n)))>::type> { \
        ASIO_STATIC_CONSTEXPR(bool, is_valid = true);                                                     \
                                                                                                          \
        using result_type = decltype(set_value(declval<T>(), ASIO_VARIADIC_DECLVAL(n)));                  \
                                                                                                          \
        ASIO_STATIC_CONSTEXPR(bool, is_noexcept = noexcept(                                               \
...
template <typename T>
struct set_value_free_trait<T, void()> : conditional<
                                             is_same<T, typename remove_reference<T>::type>::value,
                                             typename conditional<
                                                 is_same<T, typename add_const<T>::type>::value,
                                                 no_set_value_free,
                                                 traits::set_value_free<typename add_const<T>::type, void()> >::type,
                                             traits::set_value_free<typename remove_reference<T>::type, void()> >::type {
};
...
#define ASIO_PRIVATE_SET_VALUE_MEMBER_TRAIT_DEF(n)                                                                           \
    template <typename T, ASIO_VARIADIC_TPARAMS(n)>                                                                          \
    struct set_value_free_trait<T, void(ASIO_VARIADIC_TARGS(n))> : conditional<                                              \
                                                                       is_same<T, typename remove_reference<T>::type>::value \
