...
struct bulk_execute_free_trait<T, F, N,
                               typename void_type<
                                   decltype(bulk_execute(declval<T>(), declval<F>(), declval<N>()))>::type> {
    ASIO_STATIC_CONSTEXPR(bool, is_valid = true);

    using result_type = decltype(bulk_execute(declval<T>(), declval<F>(), declval<N>()));
...
template <typename T, typename F, typename N, typename = void>
struct bulk_execute_free_trait : conditional<
                                     ...
                                     typename conditional<
                                         ...
                                         traits::bulk_execute_free<typename add_const<T>::type, F, N> >::type,
