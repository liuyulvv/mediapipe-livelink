...
inline const mutable_buffer* buffer_sequence_begin(const MutableBuffer& b,
                                                   typename constraint<
                                                       is_convertible<const MutableBuffer*, const mutable_buffer*>::value>::type = 0) ASIO_NOEXCEPT {
...
inline const const_buffer* buffer_sequence_begin(const ConstBuffer& b,
                                                 typename constraint<
                                                     is_convertible<const ConstBuffer*, const const_buffer*>::value>::type = 0) ASIO_NOEXCEPT {
...
inline auto buffer_sequence_begin(C& c,
                                  typename constraint<
                                      !is_convertible<const C*, const mutable_buffer*>::value && !is_convertible<const C*, const const_buffer*>::value>::type = 0) ASIO_NOEXCEPT->decltype(c.begin()) {
    return c.begin();
}
...
inline typename C::iterator buffer_sequence_begin(C& c,
                                                  typename constraint<
                                                      !is_convertible<const C*, const mutable_buffer*>::value && !is_convertible<const C*, const const_buffer*>::value>::type = 0) ASIO_NOEXCEPT {
    return c.begin();
}
...
inline auto buffer_sequence_end(C& c,
                                typename constraint<
                                    !is_convertible<const C*, const mutable_buffer*>::value && !is_convertible<const C*, const const_buffer*>::value>::type = 0) ASIO_NOEXCEPT->decltype(c.end()) {
    return c.end();
}
...
struct buffer_sequence_cardinality : conditional<
...
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(
    ...
    return ASIO_MUTABLE_BUFFER(data.size() ? &data[0] : 0,
                               data.size() * sizeof(Elem)
                                   ...
                                   ,
                               detail::buffer_debug_check<
                                   typename std::basic_string<Elem, Traits, Allocator>::iterator>(data.begin())
#endif  // ASIO_ENABLE_BUFFER_DEBUGGING
    );
}
...
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(
    ...
    typename constraint<
        ...
        defaulted_constraint>::type = defaulted_constraint(),
    typename constraint<
        !is_const<
            typename remove_reference<
                typename std::iterator_traits<typename T::iterator>::reference>::type>::value,
        defaulted_constraint>::type = defaulted_constraint()) ASIO_NOEXCEPT {
    return ASIO_MUTABLE_BUFFER(
 ...
 */
template <typename Elem, typename Allocator>
ASIO_NODISCARD inline dynamic_vector_buffer<Elem, Allocator> dynamic_buffer(
...
std::size_t buffer_copy(one_buffer, multiple_buffers,
                        ...
                        std::size_t max_bytes_to_copy = (std::numeric_limits<std::size_t>::max)()) ASIO_NOEXCEPT {
...
std::size_t buffer_copy(multiple_buffers, multiple_buffers,
    ...
    while (total_bytes_copied != max_bytes_to_copy && target_iter != target_end && source_iter != source_end) {
        ...
        std::size_t bytes_copied = (buffer_copy_1)(target_buffer, asio::buffer(source_buffer,
