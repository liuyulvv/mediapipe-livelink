...
#if (defined(ASIO_HAS_STD_TUPLE) && defined(ASIO_HAS_DECLTYPE) && defined(ASIO_HAS_VARIADIC_TEMPLATES)) || defined(GENERATING_DOCUMENTATION)
...
public:
    ...
    auto operator()(
        ...
        ->decltype(ASIO_MOVE_CAST(Function)(this->function_)(
            ASIO_MOVE_CAST(Args)(args)...)) {
    ...
    template <typename... Args>
    auto operator()(
        ASIO_MOVE_ARG(Args)... args) const& -> decltype(Function(function_)(ASIO_MOVE_CAST(Args)(args)...)) {
...
private:
    ...
    auto invoke_helper(
        ...
        -> decltype(asio::async_initiate<CompletionToken, void(Values...)>(
            initiate(), token,
            std::get<I>(
...
public:
    ...
    template <ASIO_COMPLETION_TOKEN_FOR(void(Values...)) CompletionToken>
    auto operator()(
        ASIO_MOVE_ARG(CompletionToken) token) const& -> decltype(this->const_invoke_helper(ASIO_MOVE_CAST(CompletionToken)(token),
                                                                                           detail::index_sequence_for<Values...>())) {
...
private:
    ...
    auto const_invoke_helper(
        ASIO_MOVE_ARG(CompletionToken) token,
        detail::index_sequence<I...>) const& -> decltype(asio::async_initiate<CompletionToken, Signature>(initiation_t(initiation_), token, std::get<I>(init_args_)...)) {
        return asio::async_initiate<CompletionToken, Signature>(
...
public:
    ...
    template <ASIO_COMPLETION_TOKEN_FOR(signature) CompletionToken>
    auto operator()(
        ASIO_MOVE_ARG(CompletionToken) token) const& -> decltype(asio::async_initiate<CompletionToken, signature>(initiate(), token, head_, tail_)) {
        return asio::async_initiate<CompletionToken, signature>(
...
public:
    ...
    auto operator()(ASIO_MOVE_ARG(Args)... args) ASIO_RVALUE_REF_QUAL
        ->decltype(ASIO_MOVE_OR_LVALUE(OnTrue)(on_true_)(
            ASIO_MOVE_CAST(Args)(args)...)) {
        if (bool_) {
...
#if defined(ASIO_HAS_REF_QUALIFIED_FUNCTIONS)
    template <typename... Args>
    auto operator()(ASIO_MOVE_ARG(Args)... args) const& -> decltype(on_true_(ASIO_MOVE_CAST(Args)(args)...)) {
        if (bool_) {
    ...
    deferred_conditional<T, OnFalse> then(T on_true,
                                          typename constraint<
                                              is_deferred<T>::value>::type* = 0,
                                          typename constraint<
                                              is_same<
                                                  typename conditional<true, OnTrue, T>::type,
                                                  deferred_noop>::value>::type* = 0) ASIO_RVALUE_REF_QUAL {
...
public:
    ...
    as_default_on(ASIO_MOVE_ARG(T) object) {
        return typename decay<T>::type::template rebind_executor<
            executor_with_default<typename decay<T>::type::executor_type> >::other(ASIO_MOVE_CAST(T)(object));
    }
    ...
    typename constraint<
        ...
        deferred_function<typename decay<Function>::type> >::type
    operator()(ASIO_MOVE_ARG(Function) function) const {
    ...
    operator()(ASIO_MOVE_ARG(T) t) const {
